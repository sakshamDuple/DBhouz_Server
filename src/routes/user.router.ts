...
import { ICart, IDocument, IProduct, IUser } from "../interfaces";
...
userRouter.get("/getOne/:userId", async (req: Request, res: Response) => {
  ...
  try {
    ...
    LOG.error(error);
    res.status(500).json({
      error: `Unable to find matching document with userId: ${userId}`,
    });
  }
});

userRouter.get(
  "/getProductInventoryByProductIdAndVariant/:ProductId/:VariantName",
  async (req: Request, res: Response) => {
    const ProductId: string = req?.params?.ProductId;
    const VariantName: string = req?.params?.VariantName;
    try {
      res.status(200).json({
        inventory: await InventoryService.getByProdIdVarName(
          ProductId,
          VariantName
        ),
      });
...
userRouter.get("/dashboard/:userId", async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({
      orderTotal: (await OrderService.getByUser(userId)).length,
      totalWishList: wishList ? wishList.wishList.length : 0,
...
userRouter.put("/updateUserAddress", async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({
      update: await userService.updateAddressesForUserById(
        userId,
        addressId,
        address
      ),
...
userRouter.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(await userService.deleteSelectUserAddress(userId, addressId));
    } catch (error) {
      console.error(error);
      LOG.error(error);
      res.status(500).json({ error: error.message });
...
userRouter.post("/editProfile", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ updatedUser: await userService.editProfile(profile) });
  } catch (error) {
    console.error(error);
    LOG.error(error);
    res.status(500).json({ error: error.message });
...
userRouter.post("/getCouponByName", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ coupon: await couponService.getCouponByName(couponName) });
  } catch (error) {
    console.error(error);
    LOG.error(error);
    res.status(500).json({ error: error.message });
...
userRouter.post(
  "/getMultipleCouponByName",
  async (req: Request, res: Response) => {
    try {
      const couponName: string = req.body.couponNames;
      let theCouponArr = couponName.split(",");
      res.status(200).json({
        coupons: await couponService.getMultiCouponByNames(theCouponArr),
      });
    } catch (error) {
      console.error(error);
      LOG.error(error);
...
userRouter.post(
  "/getAllReviewsForThisUser",
  async (req: Request, res: Response) => {
    try {
      const userId: string = req.body.userId;
      res.status(200).json({
        products: await ProductService.getAllReviewOfThisUser(userId),
      });
    } catch (error) {
      console.error(error);
      LOG.error(error);
...
userRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ coupon: await couponService.getCouponById(couponId) });
    } catch (error) {
      console.error(error);
      LOG.error(error);
      res.status(500).json({ error: error.message });
...
userRouter.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({
        result: await userService.update_Cart_Wishlist(userId, cart, wishList),
      });
    } catch (error) {
      console.error(error);
      LOG.error(error);
...
userRouter.get(
  "/getCartAndWishlist/:id",
  async (req: Request, res: Response) => {
    try {
      const userId = req.params.id;
      res
        .status(200)
        .json({ result: await userService.getCartAndWishlist(userId) });
    } catch (error) {
      console.error(error);
      LOG.error(error);
      res.status(500).json({ error: error.message });
...
userRouter.delete(
  "/deleteOneWishlist/:id/:productId",
  async (req: Request, res: Response) => {
    try {
      const userId = req.params.id;
      ...
      res.status(200).json({
        result: await userService.deleteOneWishlist(userId, productId),
      });
    } catch (error) {
      console.error(error);
      LOG.error(error);
...
userRouter.post("/checkData", async (req: Request, res: Response) => {
  try {
    ...
    if (!user) throw new Error(`user ${userId} does not exist`);
    let products: IProduct[] = await ProductService.getAllByMerchant(
      userId,
      false
    );
    res
...
userRouter.get(
  "/getMultipleMerchant/action",
  async (req: Request, res: Response) => {
    const merchantIds: string = String(req?.query?.merchantId);
    let merchants = merchantIds.split(",");
    if (merchants[0] == "undefined") {
      res.status(500).json({
        error: `Unable to find matching document with given merchantId`,
      });
    }
    try {
      res.status(200).json({
        merchants: await MerchantService.getMultipleMerchant(merchants),
      });
    } catch (error) {
      LOG.error(error);
      res.status(500).json({
...
userRouter.delete("/deleteOne/:userId", async (req: Request, res: Response) => {
  try {
    ...
    if (products.length === 0) {
      ...
      res
        .status(200)
        .json({ success: `Successfully removed user with userId ${userId}` });
