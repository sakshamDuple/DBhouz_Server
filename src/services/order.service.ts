...
import { Order, OrderStatus, transactionMethod } from "../interfaces";
...
class OrderServiceClass {
  async create(newOrder: Order): Promise<Order> {
    ...
    if (newOrder.transactionDetail.transactionMethod == "CASH_ON_DELIVERY") {
      newOrder.transactionDetail.transactionMethod =
        transactionMethod.CASH_ON_DELIVERY;
    }
    newOrder = this.sanitize(newOrder);
    if (
      newOrder.transactionDetail.status == "successful" &&
      newOrder.transactionDetail.transactionMethod !=
        transactionMethod.CASH_ON_DELIVERY
    ) {
    ...
    const result: InsertOneResult<Order> = await collections.orders.insertOne(
      newOrder
    );
  ...
  async getOrderByIdSearch(
    ...
    let orders = await collections.orders
      .aggregate(agg)
      .sort({ createdAt: -1 })
      .toArray();
    console.log(orders);
...
*/
    ...
    let arr = [];
    console.log("AllProductId: ", AllProductId[0].products);
    for (let product of AllProductId)
      for (let eachProduct of product.products) arr.push(eachProduct);
  ...
  async getOrderCsvForAdmin(criterias: Object): Promise<any[]> {
    let agg = [
      {
        $match: criterias,
      },
      ...
      { $unwind: "$merchant" },
      {
        $project: {
          ...
          "Seller Name": {
            $concat: ["$merchant.firstName", " ", "$merchant.lastName"],
          },
...
*/
    ...
    await Alltransactions.forEach((transaction) => {
      ...
      let object = {
        ...
        amount_Of_Transaction:
          type == "seller" ? totalPrice : transaction.total_price,
        customerDetail: transaction.customerDetail,
  ...
  async getAdminTotalOrderForDashboard(): Promise<number> {
    return (await collections.orders.find().sort({ createdAt: -1 }).toArray())
      .length;
  }
  ...
  async getMerchantTotalOrderForDashboard(merchantId: string): Promise<number> {
    return (
      await collections.orders
        .find({ "products.sellerId": merchantId })
        .sort({ createdAt: -1 })
        .toArray()
    ).length;
  }

  async getMerchantTotalPaymentForDashboard(
    merchantId: string
  ): Promise<number> {
    let agg = [
      {
        ...
        $project: {
          products: {
            $filter: {
              ...
              as: "theseproducts",
              cond: {
                $eq: ["$$theseproducts.sellerId", merchantId],
              },
            },
          },
        },
  ...
  async getAllCustomersByMerchant(merchantId: string): Promise<number> {
    return (
      await collections.orders.distinct("customerDetail.customerId", {
        "products.sellerId": merchantId,
      })
    ).length;
  }
    ...
    if (SortByDate == "Desc") {
      ...
      return (await collections.orders
        .find({
          ...
          order_status: OrderStatus.Payment_Accepted,
        })
        .sort({ createdAt: 1 })
        .skip(Start - 1)
        .limit(PageLimit)
  ...
  async getSeller(SellerId: string): Promise<Order[]> {
    return (await collections.orders
      .find({ "products.sellerId": SellerId })
      .sort({ createdAt: -1 })
      .toArray()) as Order[];
  }
  async getSellerTotalOrder(
    SellerId: string,
    OrderType: Array<string>
  ): Promise<Order[]> {
    return (await collections.orders
  ...
  async getAllCancelledOrder(): Promise<Order[]> {
    return (await collections.orders
      .find({ order_status: OrderStatus.Cancelled })
      .sort({ createdAt: -1 })
      .toArray()) as Order[];
  }
  ...
  sanitize(o: Order): Order {
    ...
    if (o.customerDetail)
      o.customerDetail.customerId = new ObjectId(o.customerDetail.customerId);
    if (
      o.transactionDetail.transactionMethod !=
      transactionMethod.CASH_ON_DELIVERY
    )
      o.transactionDetail.transactionId = new ObjectId(
        o.transactionDetail.transactionId
      );
    if (Number.isNaN(o.total_price)) o.total_price = 0;
    if (o.createdAt)
      o.expectedDeliveryDate = 5 * 24 * 60 * 60 * 1000 + Date.now();
    return o;
  }
}
