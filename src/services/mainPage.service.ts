...
class mainPageServiceClass {
  ...
  async getAllBanner(): Promise<Banner[]> {
    return (await collections.banner
      .find({})
      .sort({ priority: 1 })
      .toArray()) as Banner[];
  }
  ...
  async createNewMainB(newBanner: Banner): Promise<Banner> {
    ...
    const existingBanner: Banner = await this.getBannerByName(
      newBanner.Banner_Title
    );
    if (existingBanner) {
      throw new Error(
        `Banner with this title name ${newBanner.Banner_Title} already exists`
      );
    }
    const result: InsertOneResult<Banner> = await collections.banner.insertOne(
      newBanner
    );
  ...
  async createMultiMainB(newBanners: Banner[]): Promise<Banner[]> {
    ...
    // )
    const result: InsertManyResult<Banner> =
      await collections.banner.insertMany(newBanners);
  ...
  async bannerUpdate(
    ...
    delete banner._id;
    let result: UpdateResult = await collections.banner.updateOne(query, {
      $set: banner,
    });
  ...
  async createOrReplaceSmallB1(newBanner: Banner): Promise<Banner> {
    ...
    await collections.banner.findOneAndDelete({
      Banner_Type: Banner_Type.Small1,
    });
    const result: InsertOneResult<Banner> = await collections.banner.insertOne(
      newBanner
    );
  ...
  async createOrReplaceSmallB2(newBanner: Banner): Promise<Banner> {
    ...
    newBanner = this.sanitizeSmallB(newBanner);
    await collections.banner.findOneAndDelete({
      Banner_Type: Banner_Type.Small2,
    });
    const result: InsertOneResult<Banner> = await collections.banner.insertOne(
      newBanner
    );
  ...
  async deleteOneBanner(BannerId: string | ObjectId): Promise<boolean> {
    ...
    const query = { _id: new ObjectId(BannerId) };
    await collections.banner.deleteOne(query);
    let mainBanner = await collections.mainPage.updateOne(
      {},
      { $pull: { MainBanner: id } }
    );
  ...
  async mainPageCreation(mainPage: MainPage): Promise<MainPage> {
    ...
    // }
    const result: InsertOneResult<MainPage> =
      await collections.mainPage.insertOne(mainPage);
  ...
  async mainPageUpdation(
    ...
    mainPage.SmallBanner1 = await this.getSmallBanner1Id();
    mainPage.SmallBanner2 = await this.getSmallBanner2Id();
    const result: UpdateResult = await collections.mainPage.updateOne(
      {},
      { $set: mainPage }
    );
    return result.modifiedCount > 0;
  ...
  async getMainPageIfAdded(): Promise<MainPage[]> {
    return (await collections.mainPage
      .find()
      .sort({ _id: -1 })
      .limit(1)
      .toArray()) as MainPage[];
